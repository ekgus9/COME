...
def apply_come_pmet_to_model(
    ...
    deltas = execute_come_pmet(model, tok, requests, hparams, cache_template=cache_template)
        ...
        for rewrite_module_name in rewrite_module_names:
            ...
            cache_fname = (
                Path(
                    str(cache_template).format(
                        str(z_layer), block_name, hparams.clamp_norm_factor, request["case_id"]
                    ) 
                )
            ...
            if (
                ...
                try:
                    ...
                    nan_indices = torch.isnan(torch.from_numpy(data["v_star"]).to("cuda"))
                    nan_indices_result = nan_indices.any()
                    if nan_indices_result:
                        data_loaded = False
                    else:
